...
class CheckoutScreenState extends State<CheckoutScreen> {
  ...
  double? _getDeliveryCharge(
    ...
    final zoneDataList = address?.zoneData ?? [];
    ...
    ZoneData zoneData = zoneDataList.firstWhere(
      ...
      orElse: () {
        if (zoneDataList.isNotEmpty) {
          if (kDebugMode) {
            print(
                'WARNING: Using fallback zone data for restaurant ${restaurant!.zoneId}');
          }
          return zoneDataList.first; // Fallback to first zone
        }
        throw Exception("No zone data available");
      },
    );
